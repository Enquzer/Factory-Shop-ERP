    ...
    if (product.agePricing && product.agePricing.length > 0) {
      for (const pricing of product.agePricing) {
        const result = await db.run(`
          INSERT INTO product_age_pricing (productId, ageMin, ageMax, sizes, price, cost)
          VALUES (?, ?, ?, ?, ?, ?)
        `, [productId, pricing.ageMin || 0, pricing.ageMax || 0, pricing.sizes || null, pricing.price, pricing.cost || 0]);
    ...
    if (product.agePricing) {
      ...
      for (const pricing of product.agePricing) {
        await db.run(`
          INSERT INTO product_age_pricing (productId, ageMin, ageMax, sizes, price, cost)
          VALUES (?, ?, ?, ?, ?, ?)
        `, [id, pricing.ageMin || 0, pricing.ageMax || 0, pricing.sizes || null, pricing.price, pricing.cost || 0]);
      }
    }
