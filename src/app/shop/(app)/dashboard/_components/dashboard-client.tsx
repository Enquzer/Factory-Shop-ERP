...
export function DashboardClient() {
    const { user } = useAuth();
    const [shopData, setShopData] = useState<{ id: string | null; error: boolean }>({ id: null, error: false });
    ...
    useEffect(() => {
        const fetchData = async () => {
            if (!user || user.role !== 'shop') {
                setShopData({ id: null, error: true });
                setLoading(false);
                return;
            }

            try {
                // Get shop ID through API
                ...
                setShopData({ id: shop.id, error: false });
    ...
    const metrics = {
      pendingOrders: orders.filter((o: any) => o.status === 'Pending' || o.status === 'Awaiting Payment').length,
      dispatchedOrders: orders.filter((o: any) => o.status === 'Dispatched').length,
