...
import { createProduct, ProductVariant } from "@/lib/products";
...
// Type for variant data when creating a new product (without id and productId which are generated by the backend)
type NewProductVariant = Omit<ProductVariant, 'id' | 'productId'>;
    ...
    try {
      ...
      const newProduct = await createProduct({
        ...
        variants: variantsWithImages.map(variant => {
          ...
          const { id, productId, ...cleanVariant } = variant as any;
          return cleanVariant;
                ...
                />
                  ...
                  render={({ field }) => (
                    <FormItem>
                      ...
                      <FormControl>
                        <Input 
                          placeholder="e.g., MC-TS-001" 
                          {...field} 
                          disabled={isLoading} 
                          onBlur={async (e) => {
                            field.onBlur(); // Handle React Hook Form blur
                            const code = e.target.value;
                            if (code && code.length >= 3) { // Minimal length check
                              try {
                                const response = await fetch(`/api/products/check-availability?code=${encodeURIComponent(code)}`);
                                if (response.ok) {
                                  ...
                                  if (data.exists) {
                                    ...
                                    toast({
                                      ...
                                      description: "The product code you entered already exists in the system.",
                                      variant: "destructive",
                                    });
                                  } else {
              ...
              <Card>
                <CardHeader>
                  <div className="flex items-center justify-between">
                    <div>
                      <CardTitle>Age-Based Pricing/Costing</CardTitle>
                      <CardDescription>Set different prices and costs for different years/ages or sizes. Use the sync button to automate.</CardDescription>
                    </div>
                    <Button
                      type="button"
                      variant="secondary"
                ...
                <CardContent className="space-y-4">
                  {agePricingFields.map((field, index) => (
                    <div key={field.id} className="grid grid-cols-1 sm:grid-cols-5 gap-4 border p-4 rounded-md relative bg-muted/20">
                      <FormField
                        control={form.control}
