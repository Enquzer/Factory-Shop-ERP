...
export function UsersProvider({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    if (!isLoading && !isLoggingIn && !redirecting) {
      if (!user) {
        // Check if we're on a shop route, store route or finance route
        ...
        const isStoreRoute = window.location.pathname.startsWith('/store');
        const isFinanceRoute = window.location.pathname.startsWith('/finance');
        const isUsersRoute = window.location.pathname.startsWith('/users');
        ...
        if (user.role === 'shop' && !isShopRoute) {
          ...
          // Factory users can access all routes except shop
          if (!isShopRoute) {
            setRedirecting(true);
  ...
  // Only factory users can access user management
